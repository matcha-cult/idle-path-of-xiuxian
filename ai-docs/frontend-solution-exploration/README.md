...
4. **已做决策 11 项、风险 8 条、开放问题 8 个**：都不阻塞开工；v1 按最保守事实实现。详见 [04-决策矩阵与风险](./04-决策矩阵与风险.md)。

5. **ionet-ts 是作者自研、无社区热度 → 现在改协议是黄金窗口**：本次探究顺手在源码里核实出 5 个 P0 级协议缺陷（其中最严重的是 `sendTo` 定向推送链路断链——README 承诺了但连接上 `userId` 全仓无赋值点，永远返回 false；广播抽象也未接线到 WS 传输）。作者视角的加固清单、最小改法与排期见 [05-框架作者视角的协议加固清单](./05-框架作者视角的协议加固清单.md)。
...
| [04-决策矩阵与风险](./04-决策矩阵与风险.md) | 11 项决策、8 条风险缓解、8 个开放问题、M0~M4 里程碑 |
| [05-框架作者视角的协议加固清单](./05-框架作者视角的协议加固清单.md) | 源码实证的 P0/P1/P2 框架改造项：定向推送断链、请求关联、判别字段、Action DI、握手鉴权、HTTP fallback；含最小改法与排期 |

---

## 建议下一步

1. **先做协议加固**（作者视角，收益最高）：按 [05](./05-框架作者视角的协议加固清单.md) §5 顺序处置 P0-4/P0-5（推送链路）→ P0-1/P0-2（关联+headers）→ P0-3（判别字段）——它们是前端并发与实时玩法成立的前提；
2. 将 [01](./01-后端通信协议实录.md) 整理为 ionet-ts 仓库的 `PROTOCOL.md`（当前协议只散落在 README 与实现里）；
3. 评审本套方案（重点：串行队列是否满足玩法预期、是否立项 reqId 升级）；
4. 按 04 §4 的 **M0 协议桩 → M1 SDK → M2 应用骨架 → M3 业务接入** 顺序开工。