# HTTP/WS 双通道与「逻辑服」拆分改造计划（v2 · M0–M5 已落地；M6 待框架仓库）
...
>
> 现状（本轮结束）：要求 1、2 均已达成，验收 A1–A8 全绿（见 §6）；本工作区对 `vendor/ionet-ts` **未做任何直接改动**，框架侧加固列入 M6（在框架仓库实施）。
...
| D4 | 是否改 `vendor/ionet-ts` | **可改，但不得从本工作区直接改**：框架改动在其所属**框架仓库**独立提交，经版本/发布流程进入本工作区（它牵涉服务端编译/部署流程）。本工作区只做 Track A 适配，框架侧需求登记为 **M6** |
| D5 | cmd 分段 | 见 §3，M0 定稿 |
...
>
> Track B：框架侧补 `ActionFactoryBeanForNest`（`05` P1-1）后可去桥接；该改动须在**框架仓库**提交（D4），本工作区只等版本升级后删除桥接。

### 4.2 WS 鉴权（v1）
`ws-server` 丢弃 headers、每请求新建 `FlowContext`、无握手鉴权。v1：JWT 放入请求 `data.__token`，自定义 `WsAuthInOut` 校验后 `ctx.bindingUserId(BigInt(id))`；受保护 Action 校验 `getUserId() !== 0n`。（Track A 兜底；标准握手鉴权属框架侧 M6，在框架仓库实施。）
...
- **框架现状**：`broadcast()` 可用；`sendTo()` 因 `ClientConnection.userId` 无赋值点而失效（`05` P0-4）；Broadcaster 抽象未接线（P0-5）。v1 由 EdgeModule 注入 `IONET_WS_SERVER` 实现广播；定向推送需框架侧 M6 补连接注册表与 user 绑定后启用，在此之前对外能力以「广播」为准。
...
| # | 风险 | 缓解 |
|---|---|---|
| R1 | Action 无 DI（框架 P1-1） | Track A：§4.1 桥接（兜底）；框架侧 M6 提供 `ActionFactoryBeanForNest` 后可去桥接 |
| R2 | WS 无 headers/握手鉴权 | Track A：token 入 `data.__token`；框架侧 M6 支持标准握手鉴权后切换 |
| R3 | 无连接 session/定向推送 | 当前仅「广播」；框架侧 M6 补连接注册表与 user 绑定后启用 `sendTo` |
