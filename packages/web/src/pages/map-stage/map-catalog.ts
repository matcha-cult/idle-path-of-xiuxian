 ...
 * 用面板上的「复制环半径」按钮导出、写回这里（三个数：外环 18 / 二环 13 / 内环 8）。
 *
 * 为什么比初值（10/9/5）好：三层真正拉开了（层间距**统一 5 格**），一眼能看出"外环—二环—内环"；
 * 外环 18 格离网格边（±21）留 3 格余量，四门落在格点 (39,21)/(21,3)/(3,21)/(21,39)。
 */
export const COURT_RING_CELLS = 8;

export const PEAK_RING_CELLS = 13;

export const GATE_RING_CELLS = 18;
