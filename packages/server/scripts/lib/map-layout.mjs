 ...
 * - `approach` → 半径 `(peaksR + gatesR) / 2`，角度 252°（任务书 §5.1 第 1 条给定的常量）。
 *   ⚠️ 实测口径：在本文件的坐标约定下 252° 落在**北偏西**（`row = C + r·sin252 < C`），
 *   于是接引区贴在**北门**一侧，与「西南偏南、贴西门」的描述不一致。这是 §5.1 给的常量、
 *   且与只读参考实现 `tmp/map-layout.mjs` 完全一致，故**按给定值实现**并在此记录差异；
 *   真要挪到西南得把角度改成 ~198°，那属于改上游规格（见任务书 §5.1）。
