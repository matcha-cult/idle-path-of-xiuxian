...
async function main(): Promise<void> {
  ...
  for (const [cmd, subCmd, name, data] of CASES) {
    const res = await wsCall({ cmd, subCmd, data: { ...data, __token: token } });
  ...
  const basesRes = await wsCall({ cmd: 30, subCmd: 3, data: { page: 1, pageSize: 1, __token: token } });
  ...
  if (baseId != null) {
    const gen = await wsCall({ cmd: 40, subCmd: 2, data: { baseId, rarity: 0, characterId, __token: token } });
    ...
    if (typeof itemId === 'number' && genOk) {
      ...
      for (const [cmd, subCmd, name] of chain) {
        const res = await wsCall({ cmd, subCmd, data: { itemId, __token: token } });
  ...
  // 鉴权拦截抽查
  const anon = await wsCall({ cmd: 30, subCmd: 1, data: {} });
  const anonCode = (anon.data as { data?: { code?: string } } | undefined)?.data?.code;
  console.log('无 token (30,1) →', JSON.stringify(anon));
  if (anonCode !== 'UNAUTHORIZED') failures.push('无 token 未被拦截');

  if (failures.length > 0) {
    console.error('\n✗ 失败项: ' + failures.join(', '));
    process.exit(1);
  }
  console.log(`\n✓ 全逻辑服冒烟通过（${CASES.length} 个只读 Action + 鉴权拦截）`);
}
