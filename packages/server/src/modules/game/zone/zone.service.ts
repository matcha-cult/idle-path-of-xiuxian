/**
 * 秘境服务（P5.1）：图鉴 / 当前秘境 / 进入 / 层数挑战
 *
 * - 战力 = realm×realmWeight + 已装备件数×equipWeight + floor(功法等级和/skillDivisor)
 * - 挑战胜利条件：playerPower ≥ basePower + (floor−1)×powerStep（确定性战力检定，P5.2 换真实战斗）
...
export class ZoneService {
  ...
  private async currentZoneId(characterId: number, realm: number): Promise<number | null> {
    ...
    const lowest = await this.gameDb.query<ZoneRow>(
      'SELECT * FROM game_zones WHERE min_realm <= $1 ORDER BY order_index, id LIMIT 1',
      [realm],
    );
    return lowest.rows[0] ? Number(lowest.rows[0].id) : null;
  }
  ...
  async catalog(userId: number): Promise<{ success: boolean; message: string; data?: unknown }> {
    ...
    const [zones, power, currentZoneId, progressRows] = await Promise.all([
      ...
      this.gameDb.query<ZoneProgressRow>('SELECT * FROM game_zone_progress WHERE character_id = $1', [character.id]),
    ]);
    const progressByZone = new Map(progressRows.rows.map((r) => [Number(r.zone_id), r]));
    const views = zones.map((z) => ({
      ...
      unlocked: character.realm >= z.min_realm,
      ...
      progress: progressOf(progressByZone.get(Number(z.id)) ?? null),
    }));
  ...
  async progress(userId: number): Promise<{ success: boolean; message: string; data?: unknown }> {
    ...
    return {
      ...
      data: {
        ...
        canChallenge: !progress.cleared && power >= req,
        isBossFloor: boss,
  ...
  async enter(userId: number, zoneCode: string): Promise<{ success: boolean; message: string; data?: unknown }> {
    ...
    if (character.realm < zone.min_realm) {
      return {
        success: false,
        message: '境界不足：进入' + zone.name + '需要 ' + zone.min_realm + ' 境',
        data: { code: 'REALM_TOO_LOW', required: zone.min_realm, current: character.realm },
      };
    }
  ...
  async challenge(userId: number, zoneCode?: string): Promise<{ success: boolean; message: string; data?: unknown }> {
    ...
    if (zoneCode) {
      zone = await this.zoneByCode(zoneCode);
      if (!zone) return fail('ZONE_NOT_FOUND', '秘境不存在：' + zoneCode);
    } else {
      const zoneId = await this.currentZoneId(character.id, character.realm);
      if (zoneId == null) return fail('ZONE_NOT_FOUND', '暂无可用秘境');
      zone = await this.zoneById(zoneId);
    }
    if (!zone) return fail('ZONE_NOT_FOUND', '秘境不存在');
    if (character.realm < zone.min_realm) {
      return {
        success: false,
        message: '境界不足：挑战' + zone.name + '需要 ' + zone.min_realm + ' 境',
        data: { code: 'REALM_TOO_LOW', required: zone.min_realm, current: character.realm },
      };
    }
    ...
    const settled = await this.unitService.settleKills(character.id, unitCode, 1, { lingyunBonusFlat: lingyunBonus });
